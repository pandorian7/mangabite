...
def save_pdf(pdf_path, image_urls):
    ...
    print(f'downloading {n_imgs} images')
    current_image = 1
    bytes_loaded = 0
    ...
    for i, obj in enumerate(link_objs):
        print(f"\r[fetching headers] ({i+1}/{n_imgs})", end="", flush=True)
        header_fetches.append(obj.response)
    print()
    for h in header_fetches:
        total_size += int(h.headers['Content-Length'])
    size = f"{total_size/1024/1024:.2f} MB"
    print("Downloading", size)
    
    Images  = []
    for i, img in enumerate(header_fetches):
        data = b""
        gen = img.iter_content(chunk_size=100*1024)
        for d in gen:
            print(f"\r[downloading images] ({i+1}/{n_imgs}) ({bytes_loaded/1024/1024:.2f}/{size}) ({bytes_loaded/total_size*100:.2f}%)", end="", flush=True)
            data += d
            bytes_loaded += 100*1024
